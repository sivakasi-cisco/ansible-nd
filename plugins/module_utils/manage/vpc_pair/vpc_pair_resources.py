...
def register_nd_state_machine_import_aliases() -> None:
    ...
    try:
        ...
        # Keep vpc_pair files importable even when state-machine framework files
        # are intentionally not present in this branch.
        return
...
HAS_ND_STATE_MACHINE = True
try:
    from ansible_collections.cisco.nd.plugins.module_utils.nd_state_machine import NDStateMachine
except Exception:
    HAS_ND_STATE_MACHINE = False

    class NDStateMachine:  # pragma: no cover - compatibility shim
...
class VpcPairResourceService:
    ...
    def execute(self, fabric_name: str) -> Dict[str, Any]:
        if not HAS_ND_STATE_MACHINE:
            raise RuntimeError(
                "nd_vpc_pair requires nd_state_machine framework files, "
                "which are not present in this branch."
            )
